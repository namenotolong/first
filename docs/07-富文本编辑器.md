  富文本编辑器使用的是[tinymce](https://www.tiny.cloud/)。它的功能实在太强大了，而且配置~~灵活~~复杂。
...
​      而且使用npm的方式除了引入tinymce.js之外，还需要导入一堆东西，当然这不重要。但是还需要额外在static中再放一份皮肤，甚至插件的问题也需要它们其放在static中来解决。根本不是直接npm i tinymce，然后import tinymce from 'tinymce/tinymce'就能完事的。
...
<script>
  ...
  export default {
    ...
    // 如果使用tinymce的页面没有被keep-alive缓存，则不需要destroyed、activated和deactivated这几个钩子。
    destroyed() {
      this.destoryTinymce();
    },
    //在缓存的页面间切换时必须先销毁编辑器实例再重新初始化。编辑器的内容通过双向绑定被保存在父组件中，初始化后父组件又会把之前的内容通过value传过来。
    activated() {
      this.init();
    },
    deactivated() {
