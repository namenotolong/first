<template>
  <div>
    ...
    <el-table :data="userList" border highlight-current-row v-loading="userTableLoading" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="gender" label="性别" width="120px" :filters="genderList" :filter-method="filter" filter-placement="bottom"></el-table-column>
      <el-table-column prop="role" label="角色" width="120px" :filters="roleList" :filter-method="filter" filter-placement="bottom"></el-table-column>
    ...
    <el-pagination class="pagination" :total="userAmount" :current-page.sync="queryCondition.currentPageNum" :page-sizes="[20, 50, 100,1000]" :page-size="queryCondition.pageSize" layout="total, sizes, prev, pager, next, jumper" background @size-change="handleSizeChange" @current-change="handleCurrentChange"></el-pagination>
      ...
      getUserList() {
        ...
        getUserList(this.queryCondition).then(res => {
          this.userList = res.data.userList.map((item, index) => {
            return {
              id: item.id,
              index: (this.queryCondition.currentPageNum - 1) * this.queryCondition.pageSize + index + 1,
              name: item.name,
              age: item.age,
              gender: item.gender,
              role: item.role,
